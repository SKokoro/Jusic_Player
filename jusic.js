...
(() => {
    ...
    const [version, homepage, thumbnailPlaceholder] = ['1.0.0', 'https://github.com/SKokoro/JUSIC', 'https://s-mohan.github.io/demo/static/img/jusic.jpg'];
     ...
     */
    const error = message => {
        throw new Error("Jusic Error：" + message);
    };
     ...
     * @param message
     */
    const log = message => win.console.log("Jusic Log：" + message);
     ...
     * @return {String} 
     */
    const __getJusicTpl = (panel = 'list') => {

        const isList = panel == 'list'
        const panelClass = isList ? 'show-list' : ''

        return `
<main class="jusic-main">
    <div class="jusic-panel">
        <div class="jusic-music-info">
            <div class="jusic-music-scroll js-jusic-scroll--title">
                <strong class="jusic-music--title js-jusic-song--title">Jusic</strong>
                <small class="jusic-music--singer js-jusic-song--singer">jusic</small>
            </div>
        </div>
        <figure class="jusic-music-thumbnail">
            <img src="${thumbnailPlaceholder}" class="js-jusic-song--thumbnail" alt="${homepage}" />
        </figure>
        <!--jusic start: music play ctrl https://Jusic.net/lab/jusic -->
        <div class="jusic-music-ctrl">
            <a class="jusic-ctrl--prev js-jusic-btn--prev" title="上一首"><i class="jusic-ico-prev"></i></a>
            <a class="jusic-ctrl--play jusic-music-play js-jusic-btn--play" title="暂停"></a>
            <a class="jusic-ctrl--next js-jusic-btn--next"><i class="jusic-ico-next" title="下一首"></i></a>
        </div>
        <!--jusic end: music ctrl https://Jusic.net/lab/jusic -->
    </div>
    <div class="jusic-panel">
        <div class="jusic-panel--scroll js-jusic-scroll--panel ${panelClass}">
            <div class="jusic-lyric--wrap">
                <ul class="jusic-lyric--scroll js-jusic-scroll--lyric"></ul>
            </div>
            <div class="jusic-list--wrap">
                <ul class="jusic-list--scroll js-jusic-scroll--list"></ul>
            </div>
        </div>
    </div>
</main>
<aside class="jusic-aside">
    <div class="jusic-ctrl jusic-ctrl--left">
        <div class="jusic-ctrl--volume">
            <a class="jusic-volume--toggle js-jusic-btn--volume"></a>
            <div class="jusic-volume--bar js-jusic-volume--bar">
                <span class="jusic-volume--value js-jusic-volume--value"></span>
                <span class="jusic-volume--slider js-jusic-volume--slider"></span>
            </div>
        </div>
        <a class="jusic-ctrl--mode jusic-mode--loop js-jusic-btn--mode" data-play-mode="1"></a>
    </div>
    <div class="jusic-progress js-jusic-progress">
        <span class="jusic-progress--buffer js-jusic-progress--buffer"></span>
        <span class="jusic-progress--value js-jusic-progress--value"><i class="jusic-progress--slider js-jusic-progress--slider"></i></span>
    </div>
    <div class="jusic-ctrl jusic-ctrl--right">
        <time class="jusic-time js-jusic-time">00:00/00:00</time>
        <a class="jusic-ctrl--lyric js-jusic-btn--lyric js-jusic-panel--tab ${!isList ? 'active' : ''}" data-panel="lyric" title="歌词"><i class="jusic-ico-lyric"></i></a>
        <a class="jusic-ctrl--list js-jusic-btn--list js-jusic-panel--tab ${isList ? 'active' : ''}" data-panel="list" title="列表"><i class="jusic-ico-list"></i></a>
    </div>
</aside>`
    }
    ...
    const _getLyric = function (playList, index, callback) {
        ...
        const cacheName = '_jusic_lyric_' + index;
        if (!lyricUrl) {
    ...
    const _createDom = (options = {}) => {
        ...
        let id = uid++;
        const jusic = doc.createElement('div');
        jusic.id = 'Jusic-jusic-' + id;
        jusic.className = 'jusic-container';
        jusic.setAttribute('data-jusic-version', version);
        jusic.setAttribute('data-jusic-homepage', homepage);
        jusic.setAttribute('data-jusic-id', id.toString());
        jusic.innerHTML = __getjusicTpl(options.panel || 'list') + '<audio class="js-jusic--audio" hidden></audio>';
        container.appendChild(jusic);
        return {
            element: jusic,
            id: id
        };
    ...
    const __domCache = function () {
        const jusic = this.jusic;
        this.dom = {
            scroll: {
                title: $('.js-jusic-scroll--title', jusic),
                panel: $('.js-jusic-scroll--panel', jusic),
                lyric: $('.js-jusic-scroll--lyric', this.panel),
                list: $('.js-jusic-scroll--list', this.panel),
            },
            song: {
                title: $('.js-jusic-song--title', jusic),
                singer: $('.js-jusic-song--singer', jusic),
                thumbnail: $('.js-jusic-song--thumbnail', jusic)
            },
            btn: {
                prev: $('.js-jusic-btn--prev', jusic),
                play: $('.js-jusic-btn--play', jusic),
                next: $('.js-jusic-btn--next', jusic),
                volume: $('.js-jusic-btn--volume', jusic),
                mode: $('.js-jusic-btn--mode', jusic),
                lyric: $('.js-jusic-btn--lyric', jusic),
                list: $('.js-jusic-btn--list', jusic)
            },
            time: $('.js-jusic-time', jusic),
            progress: {
                bar: $('.js-jusic-progress', jusic),
                buffer: $('.js-jusic-progress--buffer', this.bar),
                value: $('.js-jusic-progress--value', this.bar),
                slider: $('.js-jusic-progress--slider', this.bar)
            },
            volume: {
                bar: $('.js-jusic-volume--bar', jusic),
                value: $('.js-jusic-volume--value', this.bar),
                slider: $('.js-jusic-volume--slider', this.bar)
            }
        };
    };
    ...
    const __renderList = function () {
        ...
        data.forEach((item, index) => {
            ...
            html += `<li class="js-jusic-song--item${active}" data-song-index="${index}">
...
<span class="song-singer" title="${item.singer}">${item.singer}</span>
    ...
    const __renderLyric = function () {
        ...
        _getLyric(this.playList, this.playIndex, function (lyric) {
            if (typeof lyric === "number") {
                ...
                self.dom.scroll.lyric.innerHTML = html + `<li><a href="${homepage}" title="jusic" target="_blank">${homepage}</a></li>`;
            }
        });
    };
    ...
    const __setBuffer = function () {
        const [DOM, AUDIO, jusicId] = [this.dom, this.audio, this.jusicId];
        _bufferTimer[jusicId] && clearInterval(_bufferTimer[jusicId]);
        ...
        _bufferTimer[jusicId] = setInterval(function () {
            ...
            if (buffer > 0 && AUDIO.buffered != undefined) {
                ...
                if (Math.abs(AUDIO.duration - AUDIO.buffered.end(buffer - 1)) < 1) {
                    ...
                    clearInterval(_bufferTimer[jusicId]);
                }
            }
        }, 1e3);
    };
    ...
    const __bindAction = function () {
        ...
        //列表面板和歌词面板切换
        bind(self.jusic, 'click', '.js-jusic-panel--tab', function (event) {
            event.stopPropagation();
        ...
        bind(AUDIO, 'timeupdate', function () {
            if (!isNaN(AUDIO.duration)) {
                ...
                //歌词
                const cacheName = '_jusic_lyric_' + self.playIndex;
        ...
        bind(DOM.btn.play, 'click', function () {
            const isPlay = utils.hasClass(this, 'jusic-music-play');
            if (isPlay) {
                self.pause();
            } else {
                self.play();
        ...
        //拖动进度条
        Range(DOM.progress.bar, '.js-jusic-progress--slider', 'horizontal', range => {
        ...
        //音量拖动
        Range(DOM.volume.bar, '.js-jusic-volume--slider', 'vertical', range => {
        ...
        bind(DOM.btn.volume, 'click', function (event) {
            if (AUDIO.muted) {
                utils.removeClass(this, 'jusic-volume--mute');
                this.setAttribute('title', '静音');
                AUDIO.muted = false;
            } else {
                utils.addClass(this, 'jusic-volume--mute');
        ...
        //点击列表
        bind(DOM.scroll.list, 'click', '.js-jusic-song--item', function (event) {
    ...
    const __playMusic = function (index, callback, isInit) {
        ...
        _bufferTimer[this.jusicId] && clearInterval(_bufferTimer[this.jusicId]);
        ...
        $$('.js-jusic-song--item.active', DOM.scroll.list)
            .forEach(item => {
                item && utils.removeClass(item, 'active, pause');
            });
        const currentSong = $$('.js-jusic-song--item', DOM.scroll.list)[index];
        ...
        const [title, singer, thumbnail] = [song.title || 'jusic', song.singer || 'singer', song.thumbnail || thumbnailPlaceholder];
    ...
    class JusicMusic {
        ...
        play(callback) {
            ...
            utils.removeClass(BTN, 'jusic-music-pause');
            utils.addClass(BTN, 'jusic-music-play');
            BTN.setAttribute('title', '暂停');
            const $currentSong = $('.js-jusic-song--item.active', this.dom.scroll.list);
        ...
        pause(callback) {
            const BTN = this.dom.btn.play;
            utils.addClass(BTN, 'jusic-music-pause');
            utils.removeClass(BTN, 'jusic-music-play');
            const $currentSong = $('.js-jusic-song--item.active', this.dom.scroll.list);
        ...
        setVolume(volume = .5) {
            ...
            if (volume === 0) {
                utils.addClass(this.dom.btn.volume, 'jusic-volume--mute');
                this.dom.btn.volume.setAttribute('title', '取消静音');
                this.audio.muted = true;
            } else {
                utils.removeClass(this.dom.btn.volume, 'jusic-volume--mute');
        ...
        setMode(mode = 1) {
            ...
            switch (mode) {
                ...
                default:
                    title = '列表循环';
                    className = 'jusic-mode--loop';
                    break;
                case 2:
                    title = '随机播放';
                    className = 'jusic-mode--random';
                    break;
                case 3:
                    title = '单曲循环';
                    className = 'jusic-mode--single';
                    break;
            }
            $Mode.setAttribute('data-play-mode', mode.toString());
            $Mode.setAttribute('title', title);
            $Mode.className = $Mode.className.replace(/jusic-mode--\w+/, className);
            this.playMode = mode;
        };
        ...
        init() {
            ...
            this.jusic = create.element;
            this.jusicId = create.id;
            this.audio = this.jusic.getElementsByTagName('audio')[0];
            //缓存DOM
     ...
     * @constructor
     */
    win.jusic = (playList, config) => new JusicMusic(playList, config);
})();